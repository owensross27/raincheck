...
| Spark streaming Kafka -> `live/vp`, `live/tu` (`deploy/k8s/raincheck/streaming.yaml`) | EKS | **dark** — manifest shipped, not applied | `live/` has several writers and readers across pods, and the R2 data root is **read-only**: `paths.RemotePath` answers `exists`/`glob`/`rglob` against the store and refuses writes by design. Needs a writer ticket nobody owns yet. |
| Live MRMS precip tick, `*/5` CronJob (`deploy/k8s/raincheck/precip-live.yaml`) | EKS | **dark** — manifest shipped, not applied | same writer gate. Its `refpull` precondition cleared on 2026-08-24 (`ref/` is in the private bucket, `r2-build` exists), but with `live/precip_cell` in an emptyDir every tick is a cold start, and no other pod can read what it writes. The Mac LaunchAgent is still the only live precip tick. |
