...
def stubs(monkeypatch):
    """Record the stages and the built days instead of shelling out / starting a JVM.
    stub_stages covers BOTH process seams (ticket 07); the build stub keeps build()'s
    contract: it fills the run's `days` record, which is what the rollup behind it reads
    (ticket 06)."""
    made, built = [], []
...
def test_a_red_stage_still_leaves_the_job_running_and_exits_1(seeded, stubs, monkeypatch):
    made, built = stubs
    stub_stages(monkeypatch, made, lambda name: int(name == "gapcheck"))
...
def test_the_one_item_form_runs_that_item_and_nothing_else(seeded, monkeypatch, capsys):
    ...
    own axis bound to that item, named the way an expanded step is named. A stage with an
    argv runs that process with the item as its trailing argument - the same command its
    pod runs (tickets 06 + 07); a bare make target takes it as the axis's own variable."""
    root, _ = seeded
    ran = []
    monkeypatch.setattr(daily, "run", lambda target, **var: ran.append((target, var)) or 0)
    monkeypatch.setattr(daily, "spawn", lambda argv: ran.append(tuple(argv)) or 0)
    ...
    assert ran == [day(2)]
    ran.clear()
    daily.main(["gapfill", "vp"])            # module form: the item is the trailing argument
    assert ran == [("gapfill", "fill", "vp")]
    assert "daily: gapfill vp ok" in capsys.readouterr().out
    ran.clear()
    daily.main(["coldpush"])                 # a bare make target still goes through make
    assert ran == [("coldpush", {})]
