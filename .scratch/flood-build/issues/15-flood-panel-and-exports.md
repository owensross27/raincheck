...
- [ ] claims as fixed strings: headline "flood-report exposure rank" with the estimand `flooded_reported` named; the always-visible reporting-propensity sentence; the panel trails the storm; rank-only and uncalibrated in v1; the Window named in the tier label; the within-Cell note (live ordering inside a Cell is purely static); "a page you open during a storm, not a service that watches"; the B2-branch alternate strings selected by the shipped model id
